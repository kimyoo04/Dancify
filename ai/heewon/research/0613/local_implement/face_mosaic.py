...
def face_mosaic(videopath, savepath):
    ...
    - XXX 주의 XXX ) videopath에 무음 모자이크 처리 영상(mosaicpath)과 오디오 추출 음원(audiopath)가 함께 저장됩니다.
    ...
    mvp.ffmpeg_tools.ffmpeg_merge_video_audio(mosaicpath, audiopath, savepath, vcodec='copy', acodec='copy', ffmpeg_output=False, logger='bar')