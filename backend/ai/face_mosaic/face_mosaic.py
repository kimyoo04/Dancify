...
def face_mosaic(AWS_ACCESS_KEY_ID, AWS_SECRET_ACCESS_KEY, bucket, videoname):
    ...
    - XXX 원본 영상이 저장된 경로 : f'video/dancable_video/{videoname}.mp4' (s3_videopath)
    ...
    mvp.ffmpeg_tools.ffmpeg_merge_video_audio(mosaicpath, audiopath, local_videopath, vcodec='copy', acodec='copy', ffmpeg_output=False, logger='bar')

    #s3에 편집한 동영상 업로드하기
    s3_savepath = 'video/mosaic_video/' + videoname + '.mp4'
    client.upload_file(local_videopath, bucket, s3_savepath) #파일 업로드 : 로컬 경로, 버킷, 저장할 s3경로
